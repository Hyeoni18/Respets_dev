					...
					<div class="page-title-box">
						<div class="page-title-right">
							<form class="form-inline">
								<a href="petInsertForm?per_no=${per_no}" class="btn btn-light">반려동물
									등록</a>
							</form>
						</div>
								...
								<div class="card-deck">
									...
									<c:forEach var="petList" items="${petList}" varStatus="status">
										<div class="card d-block" style="text-align: center;">
												<a href="petInfoDetail?pet_no=${petList.pet_no}" >
													<img class="rounded-circle" id="petProfile"
													src="${petList.pet_loc}/${petList.pet_photo}"
													alt="pet profile">
												</a>
												<div class="card-body">
													<h5 class="card-title">내동물${status.count}.
														${petList.pet_name}</h5>
													<p class="card-text">
														품종 : ${petList.pet_vrt}<br>
														성별 : <c:if test="${'F' eq petList.pet_sex}">암컷</c:if>
														<c:if test="${'M' eq petList.pet_sex}">수컷</c:if><br>
